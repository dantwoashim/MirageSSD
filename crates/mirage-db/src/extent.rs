...
impl ByteExtent {
    fn validate(&self) -> Result<(), MirageError> {
        ...
        let coherent = match self.kind {
            ExtentKind::Base => {
                self.page_hash.is_some() && self.base_offset.is_some() && self.payload_id.is_none()
            }
            ExtentKind::Dirty => {
                self.payload_id.is_some() && self.page_hash.is_none() && self.base_offset.is_none()
            }
            ExtentKind::Zero => {
                self.page_hash.is_none() && self.base_offset.is_none() && self.payload_id.is_none()
            }
        };
        if !coherent {
...
/// writes for the version land or none do.
            ...
            .execute(
                "INSERT INTO byte_extents
                 (extent_id, volume_id, inode, version, start, length, kind,
                  page_hash, base_offset, payload_id, created_ns)
...
pub fn latest_version(
    ...
    connection
        .query_row(
            "SELECT MAX(version) FROM byte_extents
