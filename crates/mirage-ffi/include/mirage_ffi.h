...
/* Same with an explicit journal payload directory (the user's cache disk); null/0 keeps <state_root>/journal. */
MirageStatus mirage_engine_create_managed_drive_at(const uint16_t *index_path,size_t index_path_len,const uint16_t *state_root,size_t state_root_len,const uint16_t *object_root,size_t object_root_len,uint64_t dirty_budget_bytes,const uint16_t *drive_manifest_path,size_t drive_manifest_len,const uint16_t *repository_key_path,size_t repository_key_len,const uint16_t *journal_root,size_t journal_root_len,MirageEngineHandle **output);
