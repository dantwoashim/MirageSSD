...
int wmain(int argc, wchar_t** argv) {
    if (argc != 5 && argc != 8 && argc != 10 && argc != 12 && argc != 14) { std::wcerr << L"usage: mirage-fs <mount-directory> <mount-index> <storage-root> <owner-sid> [--cache|--managed total-bytes free-bytes [--origin path] [--drive-manifest p --repository-key p]]\n  --managed: total-bytes = advertised volume size; free-bytes = dirty-payload budget (writes beyond it fail with disk full)\n"; return 2; }
    ...
    const auto status = host.mount(mount.wstring(), argv[2], argv[3], argv[4], cache_mode || managed_mode, managed_mode, total_bytes, free_bytes, origin, drive_manifest, repository_key);
