...
void FileSystemHost::request_eviction(const std::string& bytes){std::uint64_t freed=0;if(engine_){const auto status=mirage_engine_evict_published(engine_,std::strtoull(bytes.c_str(),nullptr,10),&freed);if(status!=MIRAGE_OK){std::cerr<<"evict failed status="<<static_cast<int>(status)<<"\n";return;}}std::cout<<"MIRAGE_EVICTED "<<freed<<"\n"<<std::flush;}
