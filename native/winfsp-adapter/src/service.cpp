...
NTSTATUS FileSystemHost::run(){if(!fs_)return STATUS_INVALID_DEVICE_STATE;
    ...
    startup_stage_="recover_engine";if(engine_){const auto mark=mirage_engine_mark_mounted(engine_);startup_engine_status_=mark;if(mark!=MIRAGE_OK){std::cerr<<"recovery failed status="<<static_cast<int>(mark)<<"\n";return STATUS_UNSUCCESSFUL;}}
    startup_stage_="start_dispatcher";const auto status=FspFileSystemStartDispatcher(fs_,0);if(!NT_SUCCESS(status))return status;std::cout<<"MIRAGE_READY\n"<<std::flush;WaitForSingleObject(stop_event_,INFINITE);return STATUS_SUCCESS;}
